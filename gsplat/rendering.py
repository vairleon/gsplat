...
def rasterization_custom(
    ...
    Args:
        ...
        geometrys: The geometry attributes of the Gaussians. [N, G] (optional)
    ...
    # colors -> [C, N, D] or [nnz, D]
    if geometrys is not None:
        if packed:
            # Turn [N, D] into [nnz, D]
            geometrys = geometrys[gaussian_ids]
        else:
            # [C, N, G]
            geometrys = geometrys.repeat(C, 1, 1)  
        #
        colors = torch.cat([colors, geometrys], dim=-1)
    ...
    if colors.shape[-1] > channel_chunk:
        ...
        for i in range(n_chunks):
            ...
            render_colors_, render_alphas_, beta_, contrib_ = rasterize_to_pixels_custom(
                means2d,
                conics,
                colors_chunk,
                opacities,
                ...
                packed=packed,
                absgrad=absgrad,
                need_beta=beta is None and need_beta,
                need_contrib=contrib is None and need_contrib,
            )
            ...
            if beta is None and need_beta:
                beta = beta_
            if contrib is None and need_contrib:
                contrib = contrib_
            
        render_colors = torch.cat(render_colors, dim=-1)
        render_alphas = render_alphas[0]  # discard the rest
    else:
        render_colors, render_alphas, beta_, contrib_ = rasterize_to_pixels_custom(
            means2d,
            conics,
            colors,
            opacities,
