...
def rasterize_to_pixels_custom(
    means2d: Tensor,  # [C, N, 2] or [nnz, 2]
    conics: Tensor,  # [C, N, 3] or [nnz, 3]
    colors: Tensor,  # [C, N, channels] or [nnz, channels]
    opacities: Tensor,  # [C, N] or [nnz]
    ...
    render_colors, render_alphas, beta, contrib = _RasterizeToPixelsCustom.apply(
        means2d.contiguous(),
        conics.contiguous(),
        colors.contiguous(),
        opacities.contiguous(),
    ...
    if padded_channels > 0:
        render_colors = render_colors[..., :-padded_channels]
    return render_colors, render_alphas, beta, contrib
...
class _RasterizeToPixelsCustom(torch.autograd.Function):
    """Rasterize gaussians"""
    ...
    def backward(
        ...
        (
            ...
            v_colors,
            v_opacities,
        ) = _make_lazy_cuda_func("rasterize_to_pixels_bwd_custom")(
            means2d,
            conics,
            colors,
            opacities,
